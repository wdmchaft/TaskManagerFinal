...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kTaskManagerFinalUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kTaskManagerFinalUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
